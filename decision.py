...
#scenario stores the no.of vehicles in each lane
scenario = [[0, 0], [0, 0], [0, 0], [0, 0]]

#maintains the phase last returned by the next phase module
lastSentPhase = 0
...
#converts a urgency value to no.of seconds of the timer
...
#converts urgency value(L, M, H) to a definite value
...
#converts a value to urgency(L, M, H)
...
#fills the scenario with number of vehicles in each lane
...
#signals each slave with appropriate signal and also sends the timer value to each slave
def signalSlave(active, timer):
    try:
        ...
        print 'Some slaves are not connected'
